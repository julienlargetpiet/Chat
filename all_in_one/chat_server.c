 ...
 */
void del_from_pfds(struct pollfd pfds[], int i, int *fd_count, char (**chatrooms_name)[10]) {
  (*fd_count)--;
  memcpy((*chatrooms_name)[i], (*chatrooms_name)[*fd_count], 10);
  pfds[i] = pfds[*fd_count]; 
};

int accept_func(int newfd, int *fd_count, int *base_memory, char (**chatrooms_name)[10]) {
...
void handle_new_connection(int listener, int *fd_count,
        int *fd_size, struct pollfd **pfds, int *base_memory, char (**chatrooms_name)[10])
{
    ...
    if (newfd == -1) {
        ...
        printf("pollserver: new connection from %s on socket %d\n",
                ...
                newfd);
        if (accept_func(newfd, fd_count, base_memory, chatrooms_name) == 1) {
 ...
 */
void handle_client_data(int listener, int *fd_count, struct pollfd *pfds, int *pfd_i, char (**chatrooms_name)[10], FILE **f) {
  unsigned char tag[16];
  unsigned char iv[12];
  char response[1] = {'1'};
  unsigned char ciphertext[MAX_MSG_SIZE];
  unsigned char buf2[40] = "Someone has disconnected\0";
  char cur_chatroom_name[10];
  unsigned char key[32] = "0123456789abcdef0123456789abcdef";
  ...
  if (nbytes <= 0) { // Got error or connection closed by client
    ...
    del_from_pfds(pfds, *pfd_i, fd_count, chatrooms_name);
 ...
 */
void process_connections(int listener, int *fd_count, int *fd_size, struct pollfd **pfds, int *base_memory, char (**chatrooms_name)[10], FILE **f) {
        ...
        if ((*pfds)[i].revents & (POLLIN | POLLHUP)) {
            ...
            if ((*pfds)[i].fd == listener) {
                ...
                handle_new_connection(listener, fd_count, fd_size,
                        pfds, base_memory, chatrooms_name);
            } else {
                // Otherwise we're just a regular client
                handle_client_data(listener, fd_count, *pfds, &i, chatrooms_name, f);
            };
        };
    };
};
...
int main(void) {
  ...
  for(;;) {
      ...
      process_connections(listener, &fd_count, &fd_size, &pfds, &base_memory, &chatrooms_name, &f);
  };
