...
int accept_func(int newfd, int *fd_count, int *base_memory, char (*chatrooms_name)[10]) {
  ...
  if ((*base_memory / (sizeof(char) * 10)) - (*fd_count) - 1 < 0) {
    *base_memory *= 2;
    chatrooms_name = realloc(chatrooms_name, *base_memory);
  };
  nbytes = recv(newfd, cur_chatroom_name, 10, 0);
  printf("heeere %s\n", cur_chatroom_name);
  if (nbytes == 0) {
    printf("Connection lost for %d\n", newfd);
    close(newfd);
...
void handle_client_data(int listener, int *fd_count,
        struct pollfd *pfds, int *pfd_i, char (*chatrooms_name)[10])
{
    char buf[256];    
...
void process_connections(int listener, int *fd_count, int *fd_size,
        struct pollfd **pfds, int *base_memory, char (*chatrooms_name)[10]) {
    for(int i = 0; i < *fd_count; i++) {
        ...
        if ((*pfds)[i].revents & (POLLIN | POLLHUP)) {
            ...
            if ((*pfds)[i].fd == listener) {
                ...
                handle_client_data(listener, fd_count, *pfds, &i, chatrooms_name);
            };
        };
    };
};
...
int main(void) {
  ...
  int fd_size = 5;
  int fd_count = 0;
  int base_memory = sizeof(char) * 10 * 5;
  ...
  for(;;) {
      ...
      process_connections(listener, &fd_count, &fd_size, &pfds, &base_memory, chatrooms_name);
  };
